...
export class AutoComplete {
  ...
  conditionallyHideClearIcon(e) {
    ...
    target.parentElement.querySelector("[data-clear-input]").style.display =
      target.value ? "inline" : "none";
  }

  _attachInputEventListeners() {
    let self = this;

    this._inputEvents.attach("click", "[data-clear-input]", async function () {
    ...
    this._inputEvents.attach("submit", "form", async function (event) {
      event.preventDefault();
      event.stopPropagation();
      if (self._resultContainer) {
        const select = self._resultContainer.querySelector("li");
  ...
  async onSelect(event, closest) {
    let item = closest.classList.contains("autocomplete-item-none")
      ? null
    ...
    if (key == "Escape") {
      ...
      if (!selected) {
        if (key == "ArrowDown") {
          ...
          nextSelected =
            this._resultContainer.querySelectorAll("li:last-child")[0];
        }
      } else {
        nextSelected =
          key == "ArrowDown"
      ...
      if (resContainer) {
        let select =
          resContainer.querySelector("li.selected") ||
          resContainer.querySelector("li");
        if (select) {
          await self.onSelect(event, select);
        }
      }
